...
export default function Repos({ repos, searchRepos }: Props) {
    ...
    const filteredRepos = useMemo(() => {
        const termoNormalizado = removerAcentos(searchRepos).toLowerCase();
        // Quando nada for digitado (searchRepos) o filter() retorna true
        // Então o .filter() mantém todos os elementos de allTopics.
        return repos.filter(repo =>
            formatRepoName(repo.name).toLowerCase().includes(termoNormalizado)
        );
    }, [searchRepos, repos]);
